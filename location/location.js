...
var COORDS = {
  ...
  'yokohama':{lat:35.444,lon:139.638},'nagoya':{lat:35.183,lon:136.907},
  'sapporo':{lat:43.062,lon:141.354},'kobe':{lat:34.690,lon:135.196},
  'kyoto':{lat:35.021,lon:135.756},'fukuoka':{lat:33.590,lon:130.402},
...
function enhanceHero() {
  ...
  fetchWx(coords.lat, coords.lon, function(err, data) {
    ...
    el.innerHTML =
      '<div class="loc-hero-wx-card">'
      // Weather summary row: big icon + temp + condition
      + '<div class="loc-hero-wx-summary">'
      +   '<span class="loc-hero-wx-icon">' + (WX_ICONS[code]||'🌡') + '</span>'
      +   '<div class="loc-hero-wx-summary-right">'
      +     '<div class="loc-hero-wx-temp">' + temp + '°C</div>'
      +     '<div class="loc-hero-wx-desc">' + (WX_DESC[code]||'') + '</div>'
      +     '<div class="loc-hero-wx-feels">Feels like ' + feel + '°C</div>'
      +   '</div>'
      + '</div>'
      // Chips row
      + '<div class="loc-hero-wx-chips">'
      +   chip('💧', hum + '%', 'Humidity')
      +   chip('💨', wind + ' km/h', 'Wind')
      +   chip('☀️', '' + uv, 'UV Index')
      +   (vis  ? chip('👁', vis + ' km', 'Visibility') : '')
      +   (rise ? chip('🌅', rise, 'Sunrise') : '')
      +   (set  ? chip('🌇', set,  'Sunset')  : '')
      + '</div>'
      + '</div>';
  });
...
function chip(ico, val, lbl) {
  return '<div class="loc-hero-wx-chip">'
    + '<span class="loc-hero-chip-ico">' + ico + '</span>'
    + '<span class="loc-hero-chip-val">' + val + '</span>'
    + '<span class="loc-hero-chip-lbl">' + lbl + '</span>'
    + '</div>';
}
