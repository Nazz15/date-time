...
def fix_file(path):
    ...
    # Remove the now-dead inline <script> block that handled opening/
    # closing the dropdown menu via click/keydown listeners. Harmless to
    # leave (querySelectorAll just finds nothing now) but it's genuine
    # dead code referencing a feature that no longer exists. Some pages
    # have a leading comment before this block, others don't -- match
    # on the actual functional content (loc-menu-trigger) so both
    # variants are caught.
    DEAD_INLINE_SCRIPT_RE = re.compile(
        r'<script>\s*'
        r'(?://[^\n]*\n\s*)?'  # optional leading comment line
        r"document\.querySelectorAll\('\.loc-menu-trigger'\).*?"
        r'</script>\n?',
        re.DOTALL,
    )
    new_html = DEAD_INLINE_SCRIPT_RE.sub('', new_html)
